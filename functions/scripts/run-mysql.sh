...
then
    ...
    echo "Found existing local medthread db " $existingMySQLContainer
    docker run --name local-medthread-mysql-db -d -p 3308:3306 -e MYSQL_ROOT_HOST=% -e MYSQL_ROOT_PASSWORD=root mysql:8 
    sleep 20
    existingMySQLContainer=$(docker ps -a -q -f name="local-medthread-mysql-db")
fi
